...
* **`scraper_levantino.ipynb`**: Cuaderno de Google Colab para la extracción y raspado web automatizado de las imágenes del catálogo.
* **`preprocesamiento_y_clustering.ipynb`**: Cuaderno con el pipeline de Visión Artificial (CLAHE + Filtro Bilateral), extracción de características (ResNet50), reducción dimensional (PCA) y agrupamiento (K-Means y DBSCAN).
* **`metadatos_carl.json`**: Estructura de metadatos crudos recopilados durante el web scraping.
